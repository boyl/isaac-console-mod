...
InputSettingsUI.entries = {
  ...
  { id = "hd_font_toggle", name = "HD Font", en = "HD Font", icon = "T",
...
local CustomCommandUI = {
  addEntry = {
    ...
    icon = "+",
    tier = "A",
    desc = "Enter a raw console command, then an optional name, and save it. Advanced; use at your own risk.",
    kind = "custom_add",
    catalogAction = "add",
    canFavorite = false,
    canRemove = false,
...
function CustomCommandUI.rebuildEntries()
  ...
  for _, record in ipairs(state.customCommands.records) do
    ...
    if spec and spec.phase == "render" then
      description = "Advanced passthrough: lifecycle command; dispatched once through the safe Render path."
    elseif spec and spec.mode == "output" then
      description = "Advanced passthrough: runs as entered; inspect output in the native console or log."
    elseif spec and spec.mode == "disabled" then
      description = "Advanced passthrough (high risk): may quit, reload, change progress, or run arbitrary code."
    elseif spec then
      description = "Advanced passthrough: recognized command; runs exactly as saved."
    else
      description = "Advanced passthrough: unknown or third-party command; runs exactly as saved."
    end
    local entry = {
      id = record.id,
      customId = record.id,
...
local function saveState()
  ...
  if #payload > MAX_SAVE_BYTES then
    ...
    return false, "Storage full (" .. #payload .. "/" .. MAX_SAVE_BYTES .. " bytes); delete or shorten a custom command"
  end
...
function InputSettingsUI.refreshEntries()
  InputSettingsUI.entries[10].name = "HD Font: " .. (state.hdFontEnabled and "On" or "Off")
  InputSettingsUI.entries[10].desc = "Off by default. Uses HD fonts at the original text size and layout. Turn off for classic fonts. Your choice is saved."
...
local function queueCommand(command, requestedCount, explicitRepeatMax, trustedUnknown, rawPassthrough)
  ...
  if rawPassthrough then
    value = trim(command)
    if value == "" then
      showToast("Command cannot be empty", "warning", 120)
      return false
    end
    if #value > 120 then
      showToast("Command is too long", "warning", 120)
      return false
    end
    if value:find("\0", 1, true) then
      showToast("Command contains an invalid character", "warning", 120)
      return false
    end
...
function CustomCommandUI.confirmCommandStage()
  ...
  if value == "" then
    showToast("Command cannot be empty", "warning", 180)
    return false
  end
  if #value > 120 then
    showToast("Command is too long", "warning", 180)
    return false
  end
  if value:find("\0", 1, true) then
    showToast("Command contains an invalid character", "warning", 180)
    return false
  end
  ...
  if duplicate and duplicate.id ~= state.customEditId then
    ...
    showToast("Command already saved", "warning", 120, "Located the existing entry")
    return false
  end
  state.customDraftCommand = value
  state.customEditStage = "name"
  state.customSelectAll = state.customDraftName ~= ""
  showToast("Command confirmed", "info", 60, "Enter an optional name; Enter saves")
  return true
end
...
function CustomCommandUI.saveDraft()
  ...
  if utf8sub(name, 40) ~= name then
    showToast("Name is limited to 40 characters", "warning", 120)
    return false
  end
  if name:find("[\r\n]") then
    showToast("Name cannot contain a line break", "warning", 120)
    return false
  end
  ...
  if not ok then
    showToast("Could not save custom command", "error", 180, tostring(record))
    return false
  end
  CustomCommandUI.rebuildEntries()
  local saved, err = saveState()
  if not saved then
    ...
    CustomCommandUI.rebuildEntries()
    showToast("Custom command was not saved", "error", 180, err)
    return false
  end
  local id = record.id
  CustomCommandUI.cancelEdit()
...
function CustomCommandUI.requestDelete(entry)
  ...
  if state.customDeleteConfirmationId ~= entry.customId then
    ...
    showToast("Delete “" .. tostring(entry.name) .. "”?", "warning", 180,
      "Press Delete or right-click again")
    return false
  end
  local modelSnapshot = state.customCommands:snapshot()
  local favoriteSnapshot, orderSnapshot = {}, {}
  ...
  if not saved then
    ...
    state.favorites, state.favoriteOrder = favoriteSnapshot, orderSnapshot
    CustomCommandUI.rebuildEntries()
    showToast("Custom command was not deleted", "error", 180, err)
    return false
  end
  resetSelection()
  showToast("Custom command deleted", "success")
  return true
end
...
function Presentation.syncTypography()
  ...
  if err and not Presentation.hdWarningShown then
    ...
    showToast("HD font unavailable; using Classic this session", "warning", 240)
  end
...
local function drawText(value, x, y, role, color, boxWidth, centered)
  if fontLoaded then
    -- Measure glyphs with the active font; keep classic container and text sizes.
...
local function computeLayout(screenWidth, screenHeight)
  -- The font option preserves classic control geometry and pagination.
  ...
  local widestCategory = math.max(
    ...
    safeTextWidth(geometryBody, "CONSOLE UI")
  )
  ...
  local iconW = math.max(line10, safeTextWidth(geometryBody, "ALL"))
  ...
  local desiredSearchW = safeTextWidth(geometryBody, "Search: name/alias/ID") + pad * 3
  ...
  local repeatLabelW = math.max(safeTextWidth(geometryBody, "LB/RB"), safeTextWidth(geometryBody, "Repeat"))
...
function Presentation.hdCategoryName(category)
  return category.name or ""
end

function Presentation.iconText(value)
  return value or "?"
end
...
function Presentation.entryHintCandidates(entry, isFavorite, effectPageCount)
  ...
  elseif entry.kind == "custom_command" then
    ...
    else add("Enter: run"); add("C: edit"); add("Delete: delete") end
...
local function drawToast(screenWidth, screenHeight, hostRect)
  ...
  if state.queue and clamp(math.floor(tonumber(state.queue.total) or 1), 1, 99) > 1 then
    ...
    if hostRect then
      ...
      drawText("Running " .. done .. "/" .. total, contentX, textY, "body", TEXT.main, contentW, true)
      return
    end
...
local function drawMenu(entries, mouse)
  ...
  drawText("CONSOLE UI", sideX, sideY + L.line10 + L.pad, "body", TEXT.main, sideTextW)
  drawText("v" .. VERSION, sideX, sideY + (L.line10 + L.pad) * 2, "body", TEXT.muted, sideTextW)
  ...
  for slot = 1, CATEGORIES_PER_PAGE do
    ...
    if category then
      ...
      drawText(Presentation.iconText(category.icon), rowX + L.pad, textY, "body",
        index == state.categoryIndex and TEXT.accent or TEXT.muted, L.iconW)
      drawText(Presentation.hdCategoryName(category), rowX + L.pad * 2 + L.iconW, textY, "body", TEXT.main,
  ...
  local titleText = state.search ~= "" and "Global Search" or Presentation.hdCategoryName(category)
  ...
  if #entries == 0 and category.id == "featured" then
    drawText("No favorites yet", L.contentX, L.gridY + L.cardH, "body", TEXT.main, L.contentW, true)
    drawText(Presentation.emptyFeaturedHint(), L.contentX,
      L.gridY + L.cardH + L.line10 + L.pad, "body", TEXT.muted, L.contentW, true)
  end
  for localIndex = 1, ITEMS_PER_PAGE do
    ...
    if entry then
      ...
      drawText(Presentation.iconText(entry.icon), iconX, textY, "body",
  ...
  elseif footerMode == "custom_command" then
    ...
    local label = editingName and "Name (optional): " or "Custom command: "
    ...
    local hint = editingName and "Name may be blank · Enter saves · Esc cancels"
      or "Up/Down history · Advanced raw passthrough · Enter continues · Esc cancels"
    drawText(fittingText({ hint }, fullDetailW), fullDetailX, footerTextY + rowStep * 2, "body", TEXT.muted, fullDetailW)
    drawText("64 KiB total storage limit; an oversized change is fully rolled back",
  ...
  elseif footerMode == "entry" then
    ...
    drawText(detailTitle, fullDetailX, footerTextY + rowStep, "body", TEXT.main, fullDetailW)
    ...
    drawText("No matching entries in this view", fullDetailX, footerTextY + rowStep, "body", TEXT.warning, fullDetailW)
    drawText("Clear the search or try another term", fullDetailX, footerTextY + rowStep * 2, "body", TEXT.muted, fullDetailW)
  end
...
function Presentation.traceMenu()
  -- Trace state changes without search text, custom commands or player data.
