...
function InputSettingsUI.refreshEntries()
  ...
  InputSettingsUI.entries[9].name = "全屏光标：" .. cursor
  InputSettingsUI.entries[9].desc = "当前状态：" .. cursor .. "。点击可"
    .. (state.fullscreenCursorEnabled == false and "开启" or "关闭")
    .. "。仅在全屏控制台显示指针；若出现双光标可关闭，不改变游戏鼠标控制。"
end
...
local function registerMcmSettings()
  ...
  local closeAfterRegularCommandSetting = ModConfigMenu.OptionType.BOOLEAN ~= nil and {
    ...
    Info = {
      ...
      "关闭后保留当前页面与选中项；命令编辑会结束，但文字仍保留。",
      "回溯、重开、跳转等运行流程命令始终关闭界面。",
    },
  } or nil
  local fullscreenCursorSetting = ModConfigMenu.OptionType.BOOLEAN ~= nil and {
    ...
    Default = true,
    Display = function()
      return "全屏控制台光标: " .. (state.fullscreenCursorEnabled ~= false and "开启" or "关闭")
    end,
    OnChange = function(value)
      local ok, changed, err = InputSettingsUI.applySetting("fullscreen_cursor", value == true)
      if not ok then showToast("光标设置失败", "error", 150, err); return end
      if changed then showToast("全屏控制台光标已"
        .. (state.fullscreenCursorEnabled and "开启" or "关闭"), "success") end
    end,
    Info = {
      "只在全屏且控制台显示时绘制鼠标指针。",
      "窗口化保持原样；若出现双光标可关闭。",
  ...
  local ok, err = pcall(function()
    ...
    if closeAfterRegularCommandSetting then
      ModConfigMenu.AddSetting("Isaac Chinese Console", "设置", closeAfterRegularCommandSetting)
    end
    if fullscreenCursorSetting then
      ModConfigMenu.AddSetting("Isaac Chinese Console", "设置", fullscreenCursorSetting)
    end
  end)
  if ok then
    mcmRegistered = true
...
local function queueCommand(command, requestedCount, explicitRepeatMax, trustedUnknown, rawPassthrough)
  ...
  if state.runEndState == "game_over" and not (spec and spec.phase == "render") then
    showToast("死亡结算中无法执行普通命令", "warning", 180,
      "请使用运行控制或流程切换命令")
    return false
  end
...
function ChineseConsole.controllerShoulderOwnerActive(side, index, repeatAction, repeatButton, pageAction)
  ...
  local pageValue = ChineseConsole.controllerActionValueAt(pageAction, index)
...
local function controllerShoulderEvent(candidates)
  ...
  if event then
    ...
    local triggerEvidence = controllerButtonTriggeredAt(CONTROLLER_PAGE_PREVIOUS, event.index)
      ...
      or ChineseConsole.controllerActionValueAt(previousAction, event.index)
  ...
  if event then
    ...
    local triggerEvidence = controllerButtonTriggeredAt(CONTROLLER_PAGE_NEXT, event.index)
      ...
      or ChineseConsole.controllerActionValueAt(nextAction, event.index)
        >= CONTROLLER_INPUT_COMPATIBILITY.triggerPressThreshold
    if bumperEvidence or not triggerEvidence then
  ...
  for _, index in ipairs(candidates) do
    if ChineseConsole.controllerShoulderOwnerActive("left", index,
  ...
  for _, index in ipairs(candidates) do
    if ChineseConsole.controllerShoulderOwnerActive("right", index,
...
ChineseConsole:AddCallback(ModCallbacks.MC_GET_SHADER_PARAMS, Presentation.onLateOverlayShader)
...
ChineseConsole:AddCallback(ModCallbacks.MC_POST_GAME_END, onGameEnd)
